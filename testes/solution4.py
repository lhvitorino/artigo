...
M = 10000  # Grande o suficiente para ser maior que qualquer expressão

# Restrições para as variáveis binárias b_i e a_i usando a técnica grande-M
for i in range(12):
    # b_i = 1 se Dmi[i] > 1.05 * DC
    model += Dmi[i] - 1.05 * DC <= M * (1 - b[i])  # Se b_i = 1, a desigualdade se mantém
    model += Dmi[i] - 1.05 * DC >= 0.0001 - M * b[i]  # Se b_i = 0, Dmi[i] <= 1.05 * DC
    
    # a_i = 1 se Dmi[i] < DC
    model += DC - Dmi[i] <= M * (1 - a[i])  # Se a_i = 1, a desigualdade se mantém
    model += DC - Dmi[i] >= 0.0001 - M * a[i]  # Se a_i = 0, DC <= Dmi[i]
