...
# Dados
dMi = [3481.92, 2237.76, 2704.32, 3542.4, 3024, 2626.56, 2160, 1676.16, 2306.88, 2877.12, 2566.08, 3153.60]
n = len(dMi)
Tc = 22.16
Ts = 18.40

# Variáveis de decisão
dC1 = pulp.LpVariable('dC1', lowBound=0, cat='Integer')
dC2 = pulp.LpVariable('dC2', lowBound=0, cat='Integer')
dC3 = pulp.LpVariable('dC3', lowBound=0, cat='Integer')
dC4 = pulp.LpVariable('dC4', lowBound=0, cat='Integer')
...
# Função objetivo
prob += pulp.lpSum([z_sub[i] + z_sob[i] + z_sut[i] + z_sot[i] for i in range(n)])

# Restrições de acréscimo e decréscimos de demanda

prob += dC2 >= 1.05 * dC3 # Segundo período deve subir 5% em relação ao primeiro para configurar período de teste
...
for i in range(n):
    if i >= 0 and i <= 2:
        ...
        prob += z_sub[i] <= (dMi[i] - dC1) * 2 * Tc + M * (1 - bsub[i])  # z_sub = (dMi - dC1) * 2 * Tc * bsub
        prob += z_sub[i] >= (dMi[i] - dC1) * 2 * Tc - M * (1 - bsub[i])
        prob += z_sub[i] <= M * bsub[i]
    
        prob += z_sob[i] <= (dC1 - dMi[i]) * Ts + M * (1 - bsob[i])  # z_sob = (dC1 - dMi) * Ts * bsob
    ...
    if i >= 3 and i <= 5:
        ...
        # Definir as variáveis auxiliares
        prob += z_sut[i] <= (dMi[i] - dC2) * 2 * Tc + M * (1 - bsut[i])  # z_sut = (dMi - dC2) * 2 * Tc * bsut
        prob += z_sut[i] >= (dMi[i] - dC2) * 2 * Tc - M * (1 - bsut[i])
        prob += z_sut[i] <= M * bsut[i]
    
        prob += z_sot[i] <= (dC1 - dMi[i]) * Ts + M * (1 - bsot[i])  # z_sot = (dC1 - dMi) * Ts * bsot
    ...
    if i >= 6 and i <= 8:
        ...
        # Definir as variáveis auxiliares
        prob += z_sub[i] <= (dMi[i] - dC3) * 2 * Tc + M * (1 - bsub[i])  # z_sub = (dMi - dC3) * 2 * Tc * bsub
        prob += z_sub[i] >= (dMi[i] - dC3) * 2 * Tc - M * (1 - bsub[i])
        prob += z_sub[i] <= M * bsub[i]
    
        prob += z_sob[i] <= (dC3 - dMi[i]) * Ts + M * (1 - bsob[i])  # z_sob = (dC3 - dMi) * Ts * bsob
    ...
    if i >= 9 and i <= 11:
        ...
        # Definir as variáveis auxiliares
        prob += z_sut[i] <= (dMi[i] - dC4) * 2 * Tc + M * (1 - bsut[i])  # z_sut = (dMi - dC4) * 2 * Tc * bsut
        prob += z_sut[i] >= (dMi[i] - dC4) * 2 * Tc - M * (1 - bsut[i])
        prob += z_sut[i] <= M * bsut[i]
    
        prob += z_sot[i] <= (dC3 - dMi[i]) * Ts + M * (1 - bsot[i])  # z_sot = (dC3 - dMi) * Ts * bsot
